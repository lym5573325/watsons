				...
				$.post("Operation.jsp",jsonNames,function(data){
					if(data && data.status == "0"){
						...
						datas.map(function(item, i) {
							    ...
							    tag += "<a class=\"wea-temp16-box wea-temp-shodow\" id=" + item.id + " href=" + url + " target=\"_blank\" style=\"background:linear-gradient("+item.linearDirection+", "+item.linearStart +", "+item.linearEnd+");filter: progid:DXImageTransform.Microsoft.Gradient(startColorStr="+item.linearStart+",endColorStr="+item.linearEnd+",gradientType='1');cursor:"+cursor+" \"  >";
								tag +=    "<div class=\"wea-temp16-box-left\">";
								tag += "<p class=\"wea-temp16-box-title"+((!item.text || item.text == "") ? " wea-temp16-box-title2" : "")+"\">" + item.title + "</p>";
						...
						$(".wea-temp-shodow").css({"margin-left": parseInt((window.innerWidth - 200*size)/(size+1))});
					}
				},"json");
			});
		</script>
