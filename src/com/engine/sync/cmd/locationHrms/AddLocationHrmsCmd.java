...
public class AddLocationHrmsCmd {
    ...
    private static final String addSql = "update " + LocationHrmsBean.tableName + " set code=?,name=?,pk_areacl=?,def1=?,def2=?,city=?,province=?,country=? where id = ?";
    ...
    public boolean addLocation(LocationHrmsBean bean){
        if(StringUtils.isNotBlank(bean.getCode())){
            ...
            rs.executeUpdate(addSql, bean.getCode(), bean.getName(), bean.getPk_areacl(), bean.getDef1(), bean.getDef2(), bean.getCity(), bean.getProvince(), bean.getCountry(), billid);
